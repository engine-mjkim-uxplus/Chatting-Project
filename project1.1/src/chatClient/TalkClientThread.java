...
public class TalkClientThread extends Thread {
	...
	// 이쓰레드는 swicth문에서 전달받은 프로토콜에 맞는 메시지가 사용자 화면에 출력된다.
	public void run() {
		boolean isStop = false;
		while(!isStop) {
			try {
				String msg = "";//100#apple // 입장할 때 프로토콜 100번
				msg = (String)tc.ois.readObject(); // 톡서버쓰레드에서 넘어오는 메시지 기다리는중..
				StringTokenizer st = null;
				int protocol = 0;//100|200|201|202|500
				if(msg !=null) {
					st = new StringTokenizer(msg,"#"); // 여기서 넘어온 100#메시지내용 을 분리
					protocol = Integer.parseInt(st.nextToken()); //100. 프로토콜(Integer)을 숫자로 바꿈
				}
				switch(protocol) {
					case 100:{//100#apple 

...
public class TalkServer extends JFrame implements Runnable, ActionListener {
	...
	public void initDisplay2() {
		...
		JPanel jp_south = new JPanel();
		String cols[] = { "접속 닉네임", "IP", "접속시간", "강제종료" }; // 컬럼네임
		String data[][] = new String[0][3]; // 닉네임, ip, 접속시간
	...
	public String getTime() {
		...
		String todayTime = (h < 10 ? "0" + h + "시 " : "" + h + "시 ")  
							+ (m < 10 ? "0" + m + "분": "" + m +"분");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (obj == jbtn_log) {
			...
			// 대화내용 백업( 작성중.... ) txt파일에 어떻게 저장?
		} //else if (obj == jbtn_chat) {
//			if (obj == jbtn_log) {
//				String fileName = "chat_.txt";
//				System.out.println(fileName);// 
//				try {
//					File f = new File(logPath + fileName); // 경로 + 파일이름
//					// 파일명만 생성될 뿐 내용까지 만들어주지는 않는다.
//					// 내용부분을 담는 별도의 클래스가 필요하다.
//					WriteObject wo = new WriteObject(new BufferedWriter(// 필터클래스-카메라 필터
//							new FileWriter(f)));
//					// io패키지에는 단독으로 파일을 컨트롤할 수 있는 클래스가 있고
//					// 그 클래스에 연결해서 사용하는 필터 클래스가 존재함.(기능을 향상해줌)
//					dao = new ChatDao();
...
//		}
		...
		else if (obj == jbtn_expulsion) { /// 구현해야함. 강퇴 버튼 눌렀을 때 아무도 없을 경우
			if (globalList.size() == 0) {
				JOptionPane.showMessageDialog(this, "현재 사용자가 없습니다", "INFO", JOptionPane.INFORMATION_MESSAGE);
			} else {
